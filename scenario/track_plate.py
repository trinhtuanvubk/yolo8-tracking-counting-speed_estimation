...
from plate_recognitor import easyocr_plate_rec
...
def tracker_details(tracker_outputs, im0, plate_model, frame_idx):
    ...
    for i, box in enumerate(bbox_xyxy):
        ...
        if 0 not in cropped_obj_img_shape:
            ...
            if plate_box.shape[0]:
                ...
                number_plate = easyocr_plate_rec(im0, plate_box)
                print("PLATE: {}".format(number_plate))
            else:
                plate_box = None
                number_plate = None
        else:
        ...
        tracker_output["tracker_box"] = box.tolist()
        tracker_output["object_id"] = object_id[i]
        tracker_output["plate_box"] = plate_box
        tracker_output["number_plate"] = number_plate
...
def run(args):
    ...
    model = MultiYolo(
        model=predictor.model if 'v8' in str(
            args['yolo_model']) else args['yolo_model'],
        device=predictor.device,
        args=predictor.args
    )
    # with open("detail_tracker.json", 'a+', encoding='utf-8') as detail_tracker_json:
        ...
        for i in range(n):
            ...
            with predictor.profilers[3]:
                ...
                if predictor.tracker_outputs[i] != []:
                    predictor.detail_tracker_outputs[i] = tracker_details(
                        predictor.tracker_outputs[i], im0, plate_model, frame_idx)
                # print(predictor.detail_tracker_outputs[i])
                # with open("detail_tracker.json", 'a+', encoding='utf-8') as detail_tracker_json:
                #     json.dump(predictor.detail_tracker_outputs[i], detail_tracker_json)
            ...
            if predictor.args.verbose or predictor.args.save or predictor.args.save_txt or predictor.args.show:
                if predictor.args.only_track:
                    ...
                    s += predictor.write_results_v3(
                        i, predictor.detail_tracker_outputs, predictor.results, (p, im, im0))
            ...
            if predictor.args.save and predictor.plotted_img is not None:
                predictor.save_preds(vid_cap, i, str(
                    predictor.save_dir / p.name))
    ...
    if predictor.args.verbose and predictor.seen:
        # speeds per image
        t = tuple(x.t / predictor.seen * 1E3 for x in predictor.profilers)
    ...
    if predictor.args.save or predictor.args.save_txt or predictor.args.save_crop:
        # number of labels
        nl = len(list(predictor.save_dir.glob('labels/*.txt')))
        s = f"\n{nl} label{'s' * (nl > 1)} saved to {predictor.save_dir / 'labels'}" if predictor.args.save_txt else ''
        LOGGER.info(
            f"Results saved to {colorstr('bold', predictor.save_dir)}{s}")
